...
import (
	...
	testutil "github.com/kok-stack/cluster-kubelet/internal/test/util"
	"gotest.tools/assert"
...
func TestGetPods(t *testing.T) {
	...
	rm, err := manager.NewResourceManager(nil, podLister, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetSecret(t *testing.T) {
	...
	rm, err := manager.NewResourceManager(nil, nil, secretLister, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetConfigMap(t *testing.T) {
	...
	rm, err := manager.NewResourceManager(nil, nil, nil, configMapLister, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestListServices(t *testing.T) {
	...
	// Create a new instance of the resource manager based on the pod lister.
	rm, err := manager.NewResourceManager(nil, nil, nil, nil, serviceLister, nil)
	if err != nil {
		t.Fatal(err)
	}
