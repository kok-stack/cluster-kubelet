...
// github.com/kok-stack/cluster-kubelet/errdefs package in order for the
...
func NewPodController(cfg PodControllerConfig) (*PodController, error) {
	...
	rm, err := manager.NewResourceManager(nil, cfg.PodInformer.Lister(), cfg.SecretInformer.Lister(), cfg.ConfigMapInformer.Lister(), cfg.ServiceInformer.Lister())
	if err != nil {
